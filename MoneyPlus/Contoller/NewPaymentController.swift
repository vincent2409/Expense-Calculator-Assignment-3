...
class NewPaymentController: UITableViewController, UITextFieldDelegate {
    ...
    @IBAction func saveButtonTapped(sender: AnyObject) {
        ...
        if titleTextField.text == "" {
            ...
            let alertController = UIAlertController(title: "Oops", message: "Please enter an expense name", preferredStyle: .alert)
            alertController.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            present(alertController, animated: true, completion: nil)
        }
            // check whether amount is valid
        else if (Double(amountTextField.text!) == nil || Double(amountTextField.text!) == 0) {
